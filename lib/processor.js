        ...
        client.media.add(function(entry){
            _getSignedUrl(ctx, function(url){
                ...
                client.media.addContent(function(result){
                    ...
                    ctx.addMetadata('kaltura-id', result.id);
                    ctx.addMetadata('kaltura-dataUrl', result.dataUrl);
                    ctx.addMetadata('kaltura-thumbnail', result.thumbnailUrl);
                    callback(null);
                }, entry.id, resource);
            });
        }, mediaEntry);
...
var _getSignedUrl = function(ctx, callback) {
    ...
    RestUtil.RestRequest(ctx.tenantRestContext, versionEndpoint, 'GET', null, function(err, response) {
        ...
        var url = util.format('%s/api/download/signed?uri=%s&signature=%s&expires=%s', ctx.tenantRestContext.host, uri, sig.signature, sig.expires);
        callback(url);
    });
};
